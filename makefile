...
TEST_MAIN_FILES := tests.c

TEST_BIB_FILES := tests/unity.c

TEST_INCLUDES := tests/includes/ \

TEST_INST_DIR := tests/instructions/
TEST_INST_FILES := test_sa.c \





TEST_FILES := $(TEST_MAIN_FILES) \
			  $(TEST_BIB_FILES) \
			  $(addprefix $(TEST_INST_DIR),$(TEST_INST_FILES)) \
			  $(addprefix $(INSTRUCTION_DIR),$(INSTRUCTION_FILES)) \

TEST_OBJ_DIR := .obj_test/
TEST_OBJ_FILES := $(addprefix $(TEST_OBJ_DIR), $(TEST_FILES:.c=.o)) \
...
my_tests : makelibft tests_binary

tests_binary: $(TEST_OBJ_FILES) $(LIBFT)
	$(CC) $(CFLAGS) -I $(TEST_INCLUDES) -I $(FILES_INCLUDES) -I $(LIBFT_INCLUDES) $^ -o $@

-include $(TEST_DEP_FILES)

$(TEST_OBJ_DIR)%.o:%.c | $(TEST_OBJ_DIR)$(TEST_INST_DIR) $(TEST_OBJ_DIR)$(INSTRUCTION_DIR)
	$(CC) -c $(CFLAGS) -MMD -MP -I $(TEST_INCLUDES) -I $(FILES_INCLUDES) -I $(LIBFT_INCLUDES) $< -o $@
	
$(TEST_OBJ_DIR)$(TEST_INST_DIR):
	mkdir -p $@

$(TEST_OBJ_DIR)$(INSTRUCTION_DIR):
	mkdir -p $@
